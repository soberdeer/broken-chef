...
export default function Card({
  ...
  const fixImage = (link) => {
    if (link) {
      const replace = link?.replace('https://www.filepicker.io', '').replace('https://filepicker.io', '');
    ...
    <div className={cx(classes.card, className)}>
      <a href={href} className={classes.href}>
        ...
        <div className={classes.content}>
          <h4 className={classes.header} dangerouslySetInnerHTML={{ __html: _highlightResult?.title?.value || title }} />
          {(studio || premium) && <div className={classes.tags}>
